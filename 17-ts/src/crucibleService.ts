...
export const part1 = (inputLines: string[]): number => {
  const grid = inputLines.map(row => row.split('').map(Number))
  return findTotalHeatLoss(grid, 3, 1)
}

export const part2 = (inputLines: string[]): number => {
  const grid = inputLines.map(row => row.split('').map(Number))
  return findTotalHeatLoss(grid, 10, 4)
}

export const findTotalHeatLoss = (grid: number[][], maximumStraightSteps: number, minimumStepsInRow: number): number => {
  ...
  while (heap.length > 0) {
    ...
    for (const direction of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
      const maximumStepsTaken = entry.directionX === direction[0] && entry.directionY === direction[1] && entry.stepsMade === maximumStraightSteps
      ...
      if (nextStep.x >= 0 && nextStep.x < grid[0].length && nextStep.y >= 0 && nextStep.y < grid.length) {
        ...
        heap.push({ ...nextStep, directionX: direction[0], directionY: direction[1], totalHeatLoss: newHeatLoss, stepsMade: steps })
      }
    }
  }
  return 0
